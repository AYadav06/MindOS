...
export const Feature = () => {
  ...
  const features = [
    ...
    {
      id: 2,
      title: "Search by Concept, Not Just Keywords.",
      subtitle: "Find Anything, Instantly", 
      description: "Our AI-powered search understands what you *mean*. Ask natural questions like, 'What were those articles about AI ethics?' and MindOS finds them instantly, no matter how you saved them.",
      icon: Search,
    ...
    },
    {
      ...
      description: "Select multiple sources and ask MindOS to work with them. Get summaries, draft blog posts, compare arguments, or find hidden connections. This is where your knowledge creates new value.",
      icon: Brain,
  ...
  const BrowserExtensionDemo = ({ isActive }: { isActive: boolean }) => (
    <div className="relative bg-slate-800/50 rounded-xl p-6 backdrop-blur-sm border border-slate-700/50">
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-800 py-12 lg:py-20 px-4">
          ...
          <div className="space-y-6 lg:space-y-8">
            {features.map((feature, index) => {
              ...
              return (
                <div
                  key={feature.id}
                  className={`relative p-6 lg:p-8 rounded-2xl backdrop-blur-sm border transition-all duration-500 cursor-pointer group ${
                    isActive 
                ...
                >
                  <div className="flex items-start space-x-4 lg:space-x-6">
                    <div className={`p-3 lg:p-4 rounded-xl bg-gradient-to-r ${feature.color} shadow-lg flex-shrink-0`}>
                      <IconComponent className="w-6 h-6 lg:w-8 lg:h-8 text-white" />
                    </div>
                    
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2 lg:mb-3">
                        <h3 className="text-xs lg:text-sm font-medium text-slate-400 uppercase tracking-wider">
                          {feature.subtitle}
                        </h3>
                        <ChevronRight className={`w-4 h-4 lg:w-5 lg:h-5 text-slate-400 transition-transform duration-300 ${
                          isActive ? 'rotate-90' : 'group-hover:translate-x-1'
                        }`} />
                      </div>
                      
                      <h4 className="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-3 lg:mb-4 leading-tight">
                        {feature.title}
                      </h4>
                  ...
                  {/* Active indicator */}
                  <div className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b ${feature.color} rounded-l-2xl transition-opacity duration-500 ${
                    isActive ? 'opacity-100' : 'opacity-0'
                  }`}></div>
                </div>
              );
